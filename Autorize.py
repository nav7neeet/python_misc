...
class BurpExtender(IBurpExtender, ITab, IHttpListener, IMessageEditorController,
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        callbacks.setExtensionName("nav_Autorize")
    ...
    def initFilter(self):
        ...
        self.showAuthBypassModified = JCheckBox("Bypassed!")
        ...
        self.showAuthEnforcedModified = JCheckBox("Enforced!")
        ...
        self.showAuthBypassUnauthenticated = JCheckBox("Bypassed!")
        ...
        self.showAuthEnforcedUnauthenticated = JCheckBox("Enforced!")
    ...
    def initConfigurationTab(self):
        ...
        self.prevent304.setBounds(10, 160, 300, 30)

        self.ignore304 = JCheckBox("Ignore 304 & 204 status code")
        self.ignore304.setBounds(10, 180, 300, 30)
        self.ignore304.setSelected(True)

        self.autoScroll = JCheckBox("Auto scroll")
        self.autoScroll.setBounds(10, 220, 140, 30)

        self.doUnauthorizedRequest = JCheckBox("Check unauthenticated access")
        self.doUnauthorizedRequest.setBounds(10, 200, 300, 30)
        ...
        self.savedHeadersTitlesCombo.setBounds(315, 90, 150, 30)

        self.replaceString = JTextArea("Paste session cookie or authentication header of low privilege user. e.g.\n\ncookie: name1=value1; name2=value2;\nOR\nAuthorization: Basic YWxhZG", 5, 30)
        ...
        scrollReplaceString.setBounds(10, 10, 300, 150)

        self.fetchButton = JButton("Get cookies from last request",
                                   actionPerformed=self.fetchCookies)
        self.fetchButton.setEnabled(False)
        self.fetchButton.setBounds(220, 250, 230, 30)

        self.filtersTabs = JTabbedPane()
        self.filtersTabs.addTab("Authorization", self.EDPnl)
        self.filtersTabs.addTab("Authentication", self.EDPnlUnauth)
        self.filtersTabs.addTab("Interception Rules", self.filtersPnl)
        self.filtersTabs.addTab("Table Filter", self.filterPnl)
        self.filtersTabs.addTab("Save/Restore", self.exportPnl)

        self.filtersTabs.setSelectedIndex(0)
    ...
    def initTabs(self):
        ...
        self.logTable.getColumn("ID").setPreferredWidth(Math.round(tableWidth / 50 * 1))
        self.logTable.getColumn("Method").setPreferredWidth(Math.round(tableWidth / 50 * 1))
        self.logTable.getColumn("URL").setPreferredWidth(Math.round(tableWidth / 50 * 20))
        self.logTable.getColumn("Org Len").setPreferredWidth(Math.round(tableWidth / 50 * 2))
        self.logTable.getColumn("Mod Len").setPreferredWidth(Math.round(tableWidth / 50 * 2))
        self.logTable.getColumn("Unauth Len").setPreferredWidth(Math.round(tableWidth / 50 * 2))
        self.logTable.getColumn("Authorization").setPreferredWidth(Math.round(tableWidth / 50 * 4))
        self.logTable.getColumn("Authentication").setPreferredWidth(Math.round(tableWidth / 50 * 4))
        ...
        self.tabs.addTab("Config", self.pnl)
        
        self.tabs.addTab("Org Req", self._originalrequestViewer.getComponent())
        self.tabs.addTab("Org Resp", self._originalresponseViewer.getComponent())
        
        self.tabs.addTab("Mod Req", self._requestViewer.getComponent())
        self.tabs.addTab("Mod Res", self._responseViewer.getComponent())

        self.tabs.addTab("Unauth Req", self._unauthorizedrequestViewer.getComponent())
        self.tabs.addTab("Unauth Res", self._unauthorizedresponseViewer.getComponent())        
    ...
    def startOrStop(self, event):
        if self.startButton.getText() == "Start":
    ...
    def exportToCSV(self):
        ...
        for i in range(0,self._log.size()):
            ...
            if enforcementStatusFilter == "All Statuses":
                ...
                if ((self.showAuthBypassModified.isSelected() and "Bypassed!" == self._log.get(i)._enfocementStatus) or
                    (self.showAuthPotentiallyEnforcedModified.isSelected() and "Is enforced???" == self._log.get(i)._enfocementStatus) or
                    (self.showAuthEnforcedModified.isSelected() and "Enforced!" == self._log.get(i)._enfocementStatus) or
                    (self.showAuthBypassUnauthenticated.isSelected() and "Bypassed!" == self._log.get(i)._enfocementStatusUnauthorized) or
                    ...
                    (self.showAuthEnforcedUnauthenticated.isSelected() and "Enforced!" == self._log.get(i)._enfocementStatusUnauthorized) or
    ...
    def exportToHTML(self):
        ...
        <thead><tr><th width=\"3%\">ID</th><th width=\"5%\">Method</th><th width=\"43%\">URL</th><th width=\"9%\">Original length</th><th width=\"9%\">Modified length</th><th width=\"9%\">Unauthorized length</th><th width=\"11%\">Authorization</th><th width=\"11%\">Authorization Status</th></tr></thead>
        <tbody>"""
    ...
    def getColumnName(self, columnIndex):
        data = ['ID','Method', 'URL', 'Org Len', 'Mod Len', "Unauth Len",
...
class Table(JTable):
    ...
    def prepareRenderer(self, renderer, row, col):
        ...
        if col == 6 or col == 7:
            ...
            elif value == "Not Sure (check auth signature)" :
    ...
    def changeSelection(self, row, col, toggle, extend):
        ...
        if col == 3:
            ...
            self._extender.tabs.setSelectedIndex(4)
        elif col == 5 or col == 7:
...
class tableFilter(RowFilter):
    ...
    def include(self, entry):
        ...
        if self._extender.showAuthBypassModified.isSelected() and "Bypassed!" == entry.getValue(6):
            return True
        elif self._extender.showAuthPotentiallyEnforcedModified.isSelected() and "Not Sure (check auth signature)" == entry.getValue(6):
            return True
        elif self._extender.showAuthEnforcedModified.isSelected() and "Enforced!" == entry.getValue(6):
            return True
        elif self._extender.showAuthBypassUnauthenticated.isSelected() and "Bypassed!" == entry.getValue(7):
            return True
        elif self._extender.showAuthPotentiallyEnforcedUnauthenticated.isSelected() and "Not Sure (check auth signature)" == entry.getValue(7):
            return True
        elif self._extender.showAuthEnforcedUnauthenticated.isSelected() and "Enforced!" == entry.getValue(7):
            return True 
